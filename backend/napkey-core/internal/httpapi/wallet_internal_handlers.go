...
func (s *Server) handleReserveWallet(w http.ResponseWriter,r *http.Request){
	...
	amount:=quote.Micros;if amount<1_000_000{amount=1_000_000};if amount>s.cfg.WalletHoldMicros{writeError(w,http.StatusBadRequest,codeInvalidRequest,"request exceeds the configured wallet hold ceiling");return}
