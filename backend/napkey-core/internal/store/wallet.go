...
func (s *Store) ListTopupOrders(ctx context.Context, userID string, limit int) ([]TopupOrder, error) {
	...
	if err != nil { return nil, fmt.Errorf("store: listing top-up orders: %w", err) }
	defer rows.Close()
	...
	if err := rows.Err(); err != nil { return nil, fmt.Errorf("store: listing top-up orders: %w", err) }
	return orders, nil
}
...
func(s *Store)CreditPayOSPayment(ctx context.Context,in PayOSPaymentInput)(bool,error){
	...
	err:=s.withTx(ctx,&sql.TxOptions{Isolation:sql.LevelSerializable},func(tx *sql.Tx)error{
		...
		if order.Status==TopupPaid{duplicate=true;_,_ = tx.ExecContext(ctx,`UPDATE payment_events SET status='duplicate',matched_order_id=$2,error_message='additional PayOS transaction received for an already paid order; review for refund',processed_at=now() WHERE id=$1`,eventID,order.ID);return nil}
		...
		creditedMicros,err:=walletCreditMicros(in.AmountMicros,order.RetailVNDPerCredit);if err!=nil{return err}
...
func (s *Store) CreditPaymentEvent(ctx context.Context,eventID int64,providerTxID,memo string,amountMicros int64) error {
	...
	return s.withTx(ctx,&sql.TxOptions{Isolation:sql.LevelSerializable},func(tx *sql.Tx)error{
		...
		if errors.Is(err,sql.ErrNoRows){return ErrNotFound}; if err!=nil{return err}
		creditedMicros,err:=walletCreditMicros(amountMicros,order.RetailVNDPerCredit);if err!=nil{return err}
		if _,err=tx.ExecContext(ctx,`INSERT INTO wallets(user_id) VALUES($1) ON CONFLICT DO NOTHING`,order.UserID);err!=nil{return err}
		var balance,held int64
