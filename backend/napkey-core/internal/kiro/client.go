// Package kiro adapts the kiro-go data plane to the dataplane.Provider contract.
//
...
// signature drift would only surface at the call site, far from the cause.
var _ dataplane.Provider = (*Client)(nil)

// The contract's types are the shared vocabulary; these aliases keep kiro-go's
// wire handling below unchanged while making it speak that vocabulary.
type (
	KeyView              = dataplane.KeyView
	CreateKeyRequest     = dataplane.CreateKeyRequest
	UpdateKeyRequest     = dataplane.UpdateKeyRequest
	OperationsStatus     = dataplane.OperationsStatus
	UsageReportingStatus = dataplane.UsageReportingStatus
)

// ErrNotFound means kiro-go has no such key, which for a delete is success.
var ErrNotFound = dataplane.ErrNotFound

// ErrUnauthorized means the admin password was rejected.
var ErrUnauthorized = dataplane.ErrUnauthorized
