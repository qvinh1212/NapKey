...
| `NAPKEY_CORE_URL` | Control plane base URL for usage reporting | - |
| `NAPKEY_INTERNAL_TOKEN` | Shared secret for the control plane's `/internal/usage` | - |

### Usage reporting

When `NAPKEY_CORE_URL` and `NAPKEY_INTERNAL_TOKEN` are both set, per-request usage is
forwarded to the napkey-core control plane, which owns pricing and billing. Both are
required; a URL without a token leaves reporting off rather than sending requests that
would be rejected on every call.

Only keys provisioned by napkey-core are reported. Those carry a `napkey:<uuid>` name
prefix, which is how usage is attributed; keys created directly in this admin panel
have no control-plane row to bill and are skipped.

Reporting never blocks a request. It happens after the response is written, on a
bounded queue with retries, and each report carries a request id the control plane
deduplicates on, so a retry after a network failure cannot bill the same request
twice. If the queue overflows because the control plane is down, reports are dropped
with a counter and a warning rather than growing memory without limit — the drop is
logged because it means usage was served but will not be billed.

Per-key `rpmLimit` and `tpmLimit` are enforced in-memory with atomic minute
windows. `/admin/api/status` exposes pool capacity and usage-reporting health for
the NapKey operations dashboard.
