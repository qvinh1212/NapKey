...
func (h *Handler) runWebSearchLoop(w http.ResponseWriter, req *ClaudeRequest, thinking bool, estimatedInputTokens int, apiKeyID string, billing *billingLease) {
	...
	for roundIdx := 0; roundIdx <= maxUses; roundIdx++ {
		...
		// outputTokens comes from estimateContentBlocksTokens, so it is an estimate
		// by construction.
		h.recordSuccessForApiKey(apiKeyID, inputTokens, outputTokens, totalCredits, usageDetail{
			Billing:             billing,
			Model:               req.Model,
