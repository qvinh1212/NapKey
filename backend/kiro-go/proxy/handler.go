...
func (h *Handler) handleClaudeMessagesInternal(w http.ResponseWriter, r *http.Request) {
	...
	if hasWebSearchTool(&req) {
		h.handleWebSearchRequest(w, &req, estimatedInputTokens, apiKeyID, billing)
		return
	}
	...
	if hasWebSearchAmongTools(&req) {
		...
		h.runWebSearchLoop(w, &req, thinking, estimatedInputTokens, apiKeyID, billing)
		return
	}
	...
	if req.Stream {
		h.handleClaudeStream(w, kiroPayload, req.Model, thinking, thinkingResponseOpts, estimatedInputTokens, cacheProfile, apiKeyID, billing)
	} else {
		h.handleClaudeNonStream(w, kiroPayload, req.Model, thinking, thinkingResponseOpts, estimatedInputTokens, cacheProfile, apiKeyID, billing)
	}
}

// handleClaudeStream Claude 流式响应
func (h *Handler) handleClaudeStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, thinkingOpts claudeThinkingResponseOptions, estimatedInputTokens int, cacheProfile *promptCacheProfile, apiKeyID string, billing *billingLease) {
	...
	for attempt := 0; attempt < maxAccountRetryAttempts; attempt++ {
		...
		// Only estimate when the upstream did not tell us. Overwriting a real count
		// with an estimate would bill the customer against a guess while a measured
		...
		h.recordSuccessForApiKey(apiKeyID, inputTokens, outputTokens, credits, usageDetail{
			...
			// is the same function used for the response's usage block, so what the
			// customer is charged for matches what they were told they used.
			BillableInputTokens: billedClaudeInputTokens(inputTokens, cacheUsage),
			CacheReadTokens:     cacheUsage.CacheReadInputTokens,
...
// usageDetail carries the per-request context the napkey-core control plane needs to
// price a request. It is optional so the call sites without cache accounting (the
// OpenAI paths) can report what they do know.
type usageDetail struct {
	Billing   *billingLease
	Model     string
	AccountID string
	// BillableInputTokens is fresh, uncached input. It excludes cache reads and
	// writes, which are billed separately at very different rates. The three must
	// sum to the request's total input, or the customer pays twice for the same
	// tokens.
	BillableInputTokens int
	CacheReadTokens     int
	CacheWriteTokens    int
	LatencyMS           int64
	// OutputEstimated is true when outputTokens came from kiro-go's estimator rather
	// than from the upstream response. The control plane records the distinction so
	// an estimate is never presented to a customer as a measurement.
...
//
// The optional detail is what gets forwarded to napkey-core. It is variadic so the
// existing four-argument calls keep compiling: a caller with nothing extra to say
// still updates the local counters, it just does not report upstream.
func (h *Handler) recordSuccessForApiKey(apiKeyID string, inputTokens, outputTokens int, credits float64, detail ...usageDetail) {
...
// reportUsage forwards one request's usage to napkey-core.
//
// Fire and forget by design: the response has already been written, so this must not
// be able to slow down or fail a customer's request. The reporter queues the send and
// retries on its own.
//
...
func (h *Handler) reportUsage(apiKeyID string, outputTokens int, credits float64, d usageDetail) bool {
	...
	return reporter.Report(usageReport{
		RequestID:         usageRequestID(d),
		KeyID:             keyID,
		Model:             d.Model,
		InputTokens:       int64(maxInt(d.BillableInputTokens, 0)),
		...
		LatencyMS:         latency,
		Status:            "success",
		TokensEstimated:   d.OutputEstimated,
...
func (h *Handler) handleClaudeNonStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, thinkingOpts claudeThinkingResponseOptions, estimatedInputTokens int, cacheProfile *promptCacheProfile, apiKeyID string, billing *billingLease) {
	excluded := make(map[string]bool)
	var lastErr error
	reqStart := time.Now()
...
func (h *Handler) handleOpenAIChat(w http.ResponseWriter, r *http.Request) {
	...
	billing := billingLeaseFromContext(r.Context())
	if req.Stream {
		h.handleOpenAIStream(w, kiroPayload, req.Model, thinking, estimatedInputTokens, apiKeyID, billing)
	} else {
		h.handleOpenAINonStream(w, kiroPayload, req.Model, thinking, estimatedInputTokens, apiKeyID, billing)
	}
}

// handleOpenAIStream OpenAI 流式响应
func (h *Handler) handleOpenAIStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, estimatedInputTokens int, apiKeyID string, billing *billingLease) {
	w.Header().Set("Content-Type", "text/event-stream; charset=utf-8")
	w.Header().Set("Cache-Control", "no-cache")
	w.Header().Set("Connection", "keep-alive")
	...
	for attempt := 0; attempt < maxAccountRetryAttempts; attempt++ {
		...
		// The OpenAI-compatible path does no prompt-cache accounting, so all input
		// is billed as fresh. That over-charges relative to the Claude path on
...
func (h *Handler) handleOpenAINonStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, estimatedInputTokens int, apiKeyID string, billing *billingLease) {
	excluded := make(map[string]bool)
	var lastErr error
	reqStart := time.Now()
