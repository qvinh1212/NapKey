...
func (h *Handler) handleWebSearchRequest(w http.ResponseWriter, req *ClaudeRequest, estimatedInputTokens int, apiKeyID string, billing *billingLease) {
	...
	// Both token counts here are computed locally from the summary text, never
	// reported by an upstream, so this is always an estimate.
	h.recordSuccessForApiKey(apiKeyID, inputTokens, outputTokens, 0, usageDetail{
		Billing:             billing,
		Model:               req.Model,
		AccountID:           accountID,
