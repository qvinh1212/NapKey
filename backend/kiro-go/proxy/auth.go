...
func (h *Handler) enforceRateLimit(w http.ResponseWriter, r *http.Request, claude bool) bool {
	keyID := apiKeyIDFromContext(r.Context())
	if keyID == "" {
		return true
	}
	entry := config.GetApiKeyEntry(keyID)
	if entry == nil || (entry.RPMLimit <= 0 && entry.TPMLimit <= 0) {
		return true
	}

	_, inputTokens, maxOutputTokens, err := estimateBillingRequest(r)
	if err != nil {
		inputTokens, maxOutputTokens = 0, 0
	}
	result := h.rateLimiter.allow(keyID, entry.RPMLimit, entry.TPMLimit, int(inputTokens+maxOutputTokens))
	setRateLimitHeaders(w, result)
	if result.Allowed {
		return true
	}
	if claude {
		h.sendClaudeError(w, http.StatusTooManyRequests, "rate_limit_error", "per-minute API key limit exceeded")
	} else {
		h.sendOpenAIError(w, http.StatusTooManyRequests, "rate_limit_error", "per-minute API key limit exceeded")
	}
	return false
}
...
func setRateLimitHeaders(w http.ResponseWriter, result rateLimitResult) {
	if result.RequestLimit > 0 {
		w.Header().Set("x-ratelimit-limit-requests", strconv.Itoa(result.RequestLimit))
		w.Header().Set("x-ratelimit-remaining-requests", strconv.Itoa(result.RequestsRemaining))
		w.Header().Set("x-ratelimit-reset-requests", result.ResetAt.UTC().Format(time.RFC3339))
	}
	if result.TokenLimit > 0 {
		w.Header().Set("x-ratelimit-limit-tokens", strconv.Itoa(result.TokenLimit))
		w.Header().Set("x-ratelimit-remaining-tokens", strconv.Itoa(result.TokensRemaining))
		w.Header().Set("x-ratelimit-reset-tokens", result.ResetAt.UTC().Format(time.RFC3339))
	}
}
