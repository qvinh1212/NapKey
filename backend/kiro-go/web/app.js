...
(() => {
  ...
  async function loadStats() {
    ...
    const usage = d.usageReporting || {};
    const reportingEnabled = usage.enabled === 1;
    const reportingHealthy = usage.healthy === 1;
    $('statUsageReporting').textContent = !reportingEnabled
      ? t('stats.disabled')
      : reportingHealthy ? t('stats.healthy') : t('stats.attention');
    $('statUsageDropped').textContent = formatNum(usage.dropped || 0);
    $('statUsagePending').textContent = formatNum(usage.pending || 0);
    $('statUsageReporting').classList.toggle('stat-value--danger', reportingEnabled && !reportingHealthy);
  }
