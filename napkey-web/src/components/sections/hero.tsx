...
export function Hero() {
  ...
  return (
    <section aria-labelledby="hero-heading" className="relative overflow-hidden pt-36 pb-20 lg:pt-44 lg:pb-24">
      ...
      <div className="container-page relative">
        <div className="grid items-center gap-16 lg:grid-cols-[minmax(0,1.05fr)_minmax(28rem,0.95fr)] lg:gap-12">
          <div className="animate-rise">
            ...
            <h1
              id="hero-heading"
              className="max-w-3xl text-[3.35rem] leading-[0.94] font-semibold tracking-[-0.055em] sm:text-7xl lg:text-[5.5rem]"
            >
              <span className="block text-gradient-fade">{t('titleLine1')}</span>
              <span className="block text-gradient-fade">{t('titleLine2')}</span>
            </h1>

            <p className="mt-8 max-w-xl text-lg leading-relaxed text-muted sm:text-xl">{t('subtitle')}</p>
            ...
            <div className="relative overflow-hidden rounded-xl border border-white/15 bg-[#070908]/90 shadow-[0_32px_100px_rgba(0,0,0,0.65)] backdrop-blur">
              ...
              <div className="grid grid-cols-3 border-t border-line bg-white/[0.015]">
                {(['topUp', 'protocols', 'monthlyFee'] as const).map((key) => (
                  <div key={key} className="border-r border-line px-4 py-4 last:border-r-0 sm:px-5">
                    <p className="font-display text-xl font-semibold tracking-[-0.04em] text-fg sm:text-2xl">
                      {t(`metrics.${key}.value`)}
                    </p>
