...
function SignalPacket({ path, duration, delay }: { path: string; duration: number; delay: number }) {
  ...
  return (
    <g className="ecosystem-signal">
      <circle r="7" fill="#0086ff" opacity=".25" filter="url(#napkey-signal-glow)">
        <animateMotion path={path} dur={timing} begin={start} repeatCount="indefinite" />
      </circle>
      <circle r="2.4" fill="#67b7ff">
        <animateMotion path={path} dur={timing} begin={start} repeatCount="indefinite" />
      </circle>
    </g>
  );
...
function ClientRow({ client, name }: { client: (typeof ecosystemClients)[number]; name: string }) {
  return (
    <li className="flex min-h-14 items-center gap-3 rounded-lg border border-line bg-surface-3/70 px-3.5 py-2.5 backdrop-blur-sm">
      ...
      <span className="ml-auto size-2 shrink-0 rounded-full bg-accent shadow-[0_0_12px_rgba(0,134,255,0.55)]" aria-hidden />
    </li>
  );
}
...
function ModelRow({ model, label }: { model: (typeof ecosystemModels)[number]; label: string }) {
  return (
    <li className="flex min-h-14 items-center gap-3 rounded-lg border border-line bg-surface-3/70 px-3.5 py-2.5 backdrop-blur-sm">
      <span className="size-2 shrink-0 rounded-full bg-accent shadow-[0_0_12px_rgba(0,134,255,0.55)]" aria-hidden />
...
export function EcosystemMap() {
  ...
  return (
    <Section id="models" eyebrow={t('eyebrow')} title={t('title')} subtitle={t('subtitle')} className="border-t border-line">
      <div className="relative overflow-hidden rounded-xl border border-line bg-terminal px-4 py-8 sm:px-6 lg:px-8">
        ...
        <svg className="pointer-events-none absolute inset-0 hidden size-full lg:block" viewBox="0 0 1200 620" preserveAspectRatio="none" aria-hidden>
          <defs>
            <linearGradient id="napkey-line-left" x1="0" x2="1">
              <stop stopColor="#0086ff" stopOpacity=".18" />
              <stop offset="1" stopColor="#0086ff" stopOpacity=".72" />
            </linearGradient>
            <linearGradient id="napkey-line-right" x1="0" x2="1">
              <stop stopColor="#0086ff" stopOpacity=".72" />
              <stop offset="1" stopColor="#0086ff" stopOpacity=".18" />
            </linearGradient>
        ...
        <div className="relative grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_15rem_minmax(0,1fr)] lg:gap-14">
          ...
          <div className="flex flex-col items-center py-2 text-center">
            <div className="relative grid size-28 place-items-center rounded-[1.75rem] border border-accent/30 bg-[radial-gradient(circle_at_50%_35%,rgba(103,183,255,0.22),rgba(0,134,255,0.06)_58%,rgba(0,0,0,0)_72%)] shadow-[0_0_60px_rgba(0,134,255,0.18)]">
              <span className="ecosystem-hub-pulse absolute inset-2 rounded-[1.35rem] border border-accent/40" aria-hidden />
              <div className="grid size-16 place-items-center rounded-2xl bg-accent text-2xl font-semibold tracking-[-0.08em] text-white shadow-[0_0_28px_rgba(0,134,255,0.32)]">NK</div>
              <span className="absolute -right-1 -top-1 flex items-center gap-1 rounded-full border border-accent/30 bg-terminal px-2 py-1 font-mono text-micro text-accent-light">
