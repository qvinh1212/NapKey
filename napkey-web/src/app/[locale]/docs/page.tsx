...
export default async function DocsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main id="main" className="min-h-screen bg-bg pt-28 pb-24 sm:pt-32">
      <div className="container-page">
        <header className="border-b border-line pb-12 sm:pb-16">
          ...
          <div className="mt-5 grid gap-7 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-end">
            ...
            <div className="rounded-xl border border-line bg-terminal p-5">
        ...
        <div className="grid gap-10 pt-10 lg:grid-cols-[13rem_minmax(0,1fr)] lg:gap-14">
          <aside className="rounded-2xl border border-line bg-surface-1 px-3 py-5 lg:sticky lg:top-24 lg:h-fit">
            <p className="px-3.5 pb-2 font-mono text-label tracking-[0.08em] text-dim uppercase">{t('contents')}</p>
            <nav aria-label={t('contents')} className="flex gap-1 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible lg:pb-0">
              {sections.map((section, index) => (
                <a key={section} href={`#${section}`} className="shrink-0 rounded-[10px] px-3 py-2 text-ui text-muted transition-colors hover:bg-white/3 hover:text-fg lg:border lg:border-transparent">
          ...
          <div className="min-w-0">
            <section id="quickstart" className="scroll-mt-28 pb-16">
              ...
              <ol className="mt-8 grid gap-px overflow-hidden rounded-xl border border-line bg-line sm:grid-cols-3">
                {(['account', 'key', 'request'] as const).map((step, index) => (
                  <li key={step} className="bg-surface-1 p-5">
            ...
            <section id="api" className="scroll-mt-28 border-t border-line py-16">
              ...
              <div className="mt-8 divide-y divide-line overflow-hidden rounded-xl border border-line">
                {endpoints.map((endpoint) => (
                  <article key={endpoint} className="grid gap-3 bg-surface-1 p-5 sm:grid-cols-[13rem_1fr] sm:p-6">
              ...
              <div className="mt-6 rounded-xl border border-info/30 bg-info/10 p-5 text-ui leading-relaxed text-muted"><strong className="text-info">{t('api.authTitle')}</strong> {t('api.authBody')}</div>
            </section>
            ...
            <section id="models" className="scroll-mt-28 border-t border-line py-16">
              ...
              <ul className="mt-4 grid overflow-hidden rounded-xl border border-line sm:grid-cols-2 xl:grid-cols-3">
                {catalog.models.map((model) => <li key={model.id} className="min-w-0 border-b border-line p-4 sm:border-r"><code className="block truncate font-mono text-ui text-fg" title={model.id}>{model.id}</code><span className="mt-2 inline-flex rounded-lg border border-line bg-surface-2 px-2.5 py-1 font-mono text-micro text-muted uppercase">{model.family}</span></li>)}
              </ul>
            </section>
            ...
            <section id="limitations" className="scroll-mt-28 border-t border-line py-16">
              ...
              <div className="mt-8 divide-y divide-line overflow-hidden rounded-xl border border-warn/30 bg-warn/5">
